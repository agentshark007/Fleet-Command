...
import pygame
__version__ = '1.1.0'
__all__ = ['V', 'Key', 'Font', 'Color', 'Image', 'Sound',
           'Origin', 'Resizable', 'Window', '__version__']
    ...
    def __init__(self, file: str = None, size: int = 24):
        ...
        try:
            ...
            self.font = pygame.font.Font(
                pygame.font.get_default_font(), int(size))
...
class Window:
    ...
    def __init__(self, width: int = 800, height: int = 600, title: str = 'PGIUD Window', resizable: Resizable = Resizable.NONE, origin: Origin = Origin.BOTTOMLEFT):
        pygame.init()
        try:
            pygame.mixer.init()
        except Exception:
    ...
    def _handle_resize(self, w, h):
        if self._resizable == Resizable.NONE:
            ...
            ratio = self._original_width / \
                self._original_height if self._original_height != 0 else 1
            if w / h > ratio:
                w = int(h * ratio)
            else:
                h = int(w / ratio)
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.button == 1:
                        ...
                        self._mouse_down_secondary = False
            self._mouse_x, self._mouse_y = self._pg_to_iud(
    ...
    def clear(self, color: 'Color'):
        ...
        self._screen.fill(color.rgb_tuple() if color.a ==
                          255 else color.to_tuple())
    ...
    def fill_rounded_rect(self, a: V, b: V, color: Color, outline_thickness: int = 0, outline_color: Color = None, top_left_roundness: float = 0.0, top_right_roundness: float = 0.0, bottom_left_roundness: float = 0.0, bottom_right_roundness: float = 0.0, steps: int = 10):
        ...
        if tl > 0:
            ...
            for i in range(steps + 1):
                ...
                arc_points.append((cx + tl * math.cos(theta),
                                  cy + tl * math.sin(theta)))
        ...
        if tr > 0:
            ...
            for i in range(steps + 1):
                ...
                arc_points.append((cx + tr * math.cos(theta),
                                  cy + tr * math.sin(theta)))
        ...
        if br > 0:
            ...
            for i in range(steps + 1):
                ...
                arc_points.append((cx + br * math.cos(theta),
                                  cy + br * math.sin(theta)))
            points.extend(reversed(arc_points))
        else:
        ...
        if bl > 0:
            ...
            for i in range(steps + 1):
                ...
                arc_points.append((cx + bl * math.cos(theta),
                                  cy + bl * math.sin(theta)))
            points.extend(reversed(arc_points))
        else:
    ...
    def draw_line(self, a: V, b: V, color: 'Color', width: int = 1):
        ...
        bx, by = self._iud_to_pg(int(bx), int(by))
        if color.a == 255:
            pygame.draw.line(self._screen, color.rgb_tuple(),
                             (ax, ay), (bx, by), width)
        else:
    ...
    def fill_polygon(self, points: Iterable[Tuple[float, float]], color: 'Color', outline_thickness: int = 0, outline_color: 'Color' = None):
        ...
        if color.a == 255:
            ...
            temp = pygame.Surface((tw, th), pygame.SRCALPHA)
            shifted = [(int(round(x - min_x)), int(round(y - min_y)))
                       for x, y in pg_points]
            try:
    ...
    def draw_image(self, image: 'Image', pos: V, origin: Origin = Origin.BOTTOMLEFT, image_filter: Optional['Color'] = None, scale_x: float = 1.0, scale_y: float = 1.0, rotation: int = 0, antialiasing: bool = True):
    ...
    def draw_text(self, text: str, pos: V, font: 'Font', color: 'Color', origin: Origin = Origin.BOTTOMLEFT):
