...
def draw(self) -> None:
    self.fill_rect(V(self.screen_left, self.screen_bottom), V(self.screen_right, self.screen_top), color=self.mainmenu_background_color)
...
def draw_button(self, text, index, max_index):
    ...
    button_color = self.mainmenu_button_color_hover if mouse_in_area(*self.mouse_pos.to_tuple(), left, right, bottom, top) else self.mainmenu_button_color
    self.fill_rounded_rect(V(left, bottom), V(right, top), color=button_color, outline_thickness=self.mainmenu_button_outline_thickness * self.gui_scale, outline_color=self.mainmenu_button_outline_color, top_left_roundness=self.mainmenu_button_roundness * self.gui_scale, top_right_roundness=self.mainmenu_button_roundness * self.gui_scale, bottom_left_roundness=self.mainmenu_button_roundness * self.gui_scale, bottom_right_roundness=self.mainmenu_button_roundness * self.gui_scale)
    self.draw_text(text, pos=V((left + right) / 2, (bottom + top) / 2), font=self.context_font.new_size(int(20 * self.gui_scale)), color=Color(255, 255, 255), origin=Origin.CENTER)
