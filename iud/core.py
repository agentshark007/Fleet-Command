...
def load_assets(self) -> None:
    ...
    self.title_font = Font(asset.asset(
        'fonts/BlackOpsOne-Regular.ttf'), size=32)
    self.context_font = Font(asset.asset(
        'fonts/WDXLLubrifontSC-Regular.ttf'), size=16)
    ...
    self.projectile_images = [Image(asset.asset('images/projectile/projectile_0.png')), Image(asset.asset(
        'images/projectile/projectile_1.png')), Image(asset.asset('images/projectile/projectile_2.png'))]
...
def handle_gui_scaling(self) -> None:
    ...
    if command_down:
        if self.keydown(Key.EQUALS) and (not self.plus_last_frame):
            ...
            log.info(
                f'GUI scale increased: old={old_gui_scale}, new={self.gui_scale}')
        elif self.keydown(Key.MINUS) and (not self.minus_last_frame):
            old_gui_scale = self.gui_scale
            self.gui_scale /= self.gui_scale_factor
            log.info(
                f'GUI scale decreased: old={old_gui_scale}, new={self.gui_scale}')
