...
def initialize_game_logic(self):
    ...
    self.units = {}
    for i in range(20):
        possible_units = [Battleship(team_index=random.randint(0, len(self.teams) - 1), position_x=random.uniform(-3000, 3000), position_y=random.uniform(-3000, 3000), direction=random.uniform(0, 360)), Warship(team_index=random.randint(0, len(self.teams) - 1), position_x=random.uniform(-3000, 3000), position_y=random.uniform(-3000, 3000), direction=random.uniform(0, 360))]
...
def handle_unit_control(self):
    ...
    def manual_override():
        return any([self.keydown(Key.W), self.keydown(Key.S), self.keydown(Key.A), self.keydown(Key.D)])
    for unit_id, unit in self.units.items():
        team = self.teams[unit.team_index]
        if team.type == TeamType.PLAYER:
            if unit_id in self.selected_units_ids:
                if self.mouse_down_secondary:
                    mouse_world_x, mouse_world_y = self.camera.deduce(self.mouse_pos.x, self.mouse_pos.y)
                    unit.autonomous = True
                    unit.autonomous_target_x = mouse_world_x
                    unit.autonomous_target_y = mouse_world_y
                    log.info(f"autonomous_target_set unit_id={getattr(unit, 'unit_id', unit_id)} target=({unit.autonomous_target_x: .1f}, {unit.autonomous_target_y: .1f})")
                if manual_override():
            ...
            log.warn(f'unknown_team_type unit_id={unit_id} team_index={unit.team_index}')
        if getattr(unit, 'autonomous', False):
            ...
            if abs(angle_diff) < self.autonomous_forward_backward_angle_threshold:
                rotation_change = max(-unit.rotation_speed, min(unit.rotation_speed, angle_diff))
                acceleration = unit.speed
            else:
                rotation_change = max(-unit.rotation_speed, min(unit.rotation_speed, angle_diff))
                acceleration = -unit.speed
            acceleration = max(-unit.speed, min(unit.speed, acceleration))
            rotation_change = max(-unit.rotation_speed, min(unit.rotation_speed, rotation_change))
            ...
            if team.type == TeamType.PLAYER:
                if unit_id in self.selected_units_ids:
                    if self.keydown(Key.SPACE):
                        mouse_world_pos = self.camera.deduce(self.mouse_pos.x, self.mouse_pos.y)
                        direction = calculate_direction(unit.position_x, unit.position_y, *mouse_world_pos)
                        projectile = Missile(x=unit.position_x, y=unit.position_y, direction=direction, shooter_id=unit.team_index)
                        pid = self.next_projectile_id
                        self.projectiles[pid] = projectile
                        log.info(f'projectile_created id={pid} type=Missile shooter_team={unit.team_index} pos=({unit.position_x: .1f}, {unit.position_y: .1f}) dir={direction: .1f}')
...
def handle_ai_for_unit(self, unit_id, unit):
    ...
    if unit.ai_state == 'WANDER':
        ...
        if enemy is None:
            ...
            predict_x = enemy.position_x + getattr(enemy, 'velocity_x', 0) * lead_seconds
            predict_y = enemy.position_y + getattr(enemy, 'velocity_y', 0) * lead_seconds
            unit.autonomous = True
    ...
    for unit_id, unit in self.units.items():
        for projectile_id, projectile in self.projectiles.items():
            ...
            if dist < unit.collision_radius:
                ...
                log.info(f'hit projectile_id={projectile_id} target_unit={unit_id} damage={projectile.damage} unit_health_after={unit.health}')
                if unit.health <= 0:
                    units_to_remove.add(unit_id)
                    log.info(f'unit_destroyed unit_id={unit_id} team={unit.team_index} pos=({unit.position_x: .1f}, {unit.position_y: .1f})')
    ...
    for unit_id_a, unit_a in self.units.items():
        for unit_id_b, unit_b in self.units.items():
            ...
            if min_dist > dist > 0:
                ...
                log.info(f'unit_collision unit_a={unit_id_a} unit_b={unit_id_b} contact_pos=({(unit_a.position_x + unit_b.position_x) / 2: .1f}, {(unit_a.position_y + unit_b.position_y) / 2: .1f})')
                create_explosion(self, (unit_a.position_x + unit_b.position_x) / 2, (unit_a.position_y + unit_b.position_y) / 2)
...
def handle_camera_movement(self):
        ...
        if self.keydown(Key.EQUALS) and (not self.plus_last_frame):
            ...
            if self.camera.scale != old_scale:
                ...
                if self.camera.scale != attempted:
                    log.warn(f'camera_zoom_clamped attempted={attempted: .2f} clamped_to={self.camera.scale: .2f}')
    ...
    if closest_unit_index_selectable == -1:
        if self.mouse_down_primary:
            ...
            self.camera.x += (self.mouse_last_frame.x - self.mouse_pos.x) / self.camera.scale
            self.camera.y += (self.mouse_last_frame.y - self.mouse_pos.y) / self.camera.scale
...
def draw_units(self):
    ...
    for unit_id, unit in self.units.items():
        ...
        if unit_id in self.selected_units_ids:
            self.draw_image(unit.image, pos=V(screen_x, screen_y), origin=Origin.CENTER, scale_x=0.5 * self.camera.scale, scale_y=0.5 * self.camera.scale, image_filter=self.selected_unit_filter, rotation=unit.direction)
            if unit.autonomous:
                target_screen_x, target_screen_y = self.camera.project(unit.autonomous_target_x, unit.autonomous_target_y)
                self.draw_image(self.autonomous_target_image, pos=V(target_screen_x, target_screen_y), origin=Origin.CENTER, scale_x=self.autonomous_target_image_scale * self.camera.scale, scale_y=self.autonomous_target_image_scale * self.camera.scale, rotation=0)
        elif unit_id == closest_unit_index_selectable:
            self.draw_image(unit.image, pos=V(screen_x, screen_y), origin=Origin.CENTER, scale_x=0.5 * self.camera.scale, scale_y=0.5 * self.camera.scale, image_filter=self.hover_unit_filter, rotation=unit.direction)
            if self.teams[self.units[closest_unit_index_selectable].team_index].type == TeamType.PLAYER:
                if unit.autonomous:
                    target_screen_x, target_screen_y = self.camera.project(unit.autonomous_target_x, unit.autonomous_target_y)
                    self.draw_image(self.autonomous_target_image, pos=V(target_screen_x, target_screen_y), origin=Origin.CENTER, scale_x=self.autonomous_target_image_scale * self.camera.scale, scale_y=self.autonomous_target_image_scale * self.camera.scale, rotation=0)
        else:
            self.draw_image(unit.image, pos=V(screen_x, screen_y), origin=Origin.CENTER, scale_x=0.5 * self.camera.scale, scale_y=0.5 * self.camera.scale, image_filter=self.other_unit_filter, rotation=unit.direction)
    for unit_id, unit in self.units.items():
        screen_x, screen_y = self.camera.project(unit.position_x, unit.position_y)
        self.draw_image(self.selection_marker_image, pos=V(screen_x, screen_y + self.selection_marker_offset * self.camera.scale), origin=Origin.BOTTOM, scale_x=self.selection_marker_scale * self.camera.scale, scale_y=self.selection_marker_scale * self.camera.scale, image_filter=self.teams[unit.team_index].color, rotation=0)
...
def draw_projectiles(self):
    for projectile in self.projectiles.values():
        ...
        if self.projectile_images:
            ...
            self.draw_image(img, pos=V(screen_x, screen_y), origin=Origin.CENTER, scale_x=1 * self.camera.scale, scale_y=1 * self.camera.scale, image_filter=Color(255, 255, 255, 255), rotation=90 - projectile.direction)

def draw_explosions(self):
    for explosion_id, explosion in self.explosions.items():
        screen_x, screen_y = self.camera.project(explosion.x, explosion.y)
        img = explosion.image()
        self.draw_image(img, pos=V(screen_x, screen_y), origin=Origin.CENTER, scale_x=explosion.scale * self.camera.scale, scale_y=explosion.scale * self.camera.scale, image_filter=Color(255, 255, 255, 255), rotation=0)
...
def draw_water(self):
    ...
    draw_water_layer(self, Color(30, 60, 170, 255), Color(20, 20, 20, 0), Color(51, 51, 51, 0), offset_x_0, offset_y_0, False)
    wave_speed_1 = 1.5
    offset_x_1 = self.water_state * wave_speed_1
    offset_y_1 = -self.water_state * wave_speed_1 * 0.8
    draw_water_layer(self, Color(70, 170, 230, 120), Color(15, 15, 15, 0), Color(26, 26, 26, 0), offset_x_1, offset_y_1, True)

def draw_water_layer(self, color: Color, color_fluctuation_strength: Color, color_fluctuation_speed: Color, offset_x: float=0.0, offset_y: float=0.0, per_tile_offset: bool=False):
    final_color = Color(color.r + math.sin(color_fluctuation_speed.r * self.water_state) * color_fluctuation_strength.r, color.g + math.cos(color_fluctuation_speed.g * self.water_state) * color_fluctuation_strength.g, color.b + math.sin(color_fluctuation_speed.b * self.water_state) * color_fluctuation_strength.b, color.a + math.cos(color_fluctuation_speed.a * self.water_state) * color_fluctuation_strength.a)
...
def draw_tiled_water(self, filter_color: Color, offset_x: float=0.0, offset_y: float=0.0, per_tile_offset: bool=False):
    ...
    for col in range(cols):
        for row in range(rows):
            ...
            self.draw_image(self.water_image, pos=V(sx, sy), origin=Origin.BOTTOMLEFT, scale_x=scalex, scale_y=scaley, image_filter=filter_color, rotation=0)

def draw_ui_panels(self):
    self.fill_rounded_rect(V(self.screen_left, self.screen_bottom), V(self.extend(self.screen_left, 150, ExtendDirection.RIGHT), self.extend(self.screen_bottom, 100, ExtendDirection.UP)), color=self.side_panel_color, outline_thickness=self.panel_outline_thickness * self.gui_scale, outline_color=self.panel_outline_color, top_left_roundness=0, top_right_roundness=self.side_panel_roundness * self.gui_scale, bottom_left_roundness=0, bottom_right_roundness=0)
    self.fill_rounded_rect(V(self.extend(self.screen_right, 150, ExtendDirection.LEFT), self.screen_bottom), V(self.screen_right, self.extend(self.screen_bottom, 100, ExtendDirection.UP)), color=self.side_panel_color, outline_thickness=self.panel_outline_thickness * self.gui_scale, outline_color=self.panel_outline_color, top_left_roundness=self.side_panel_roundness * self.gui_scale, top_right_roundness=0, bottom_left_roundness=0, bottom_right_roundness=0)
    self.fill_rect(V(self.extend(self.screen_left, 150, ExtendDirection.RIGHT), self.screen_bottom), V(self.extend(self.screen_right, 150, ExtendDirection.LEFT), self.extend(self.screen_bottom, 80, ExtendDirection.UP)), color=self.middle_panel_color, outline_thickness=self.panel_outline_thickness * self.gui_scale, outline_color=self.panel_outline_color)
    self.fill_rect(V(self.screen_left, self.screen_top), V(self.screen_right, self.extend(self.screen_top, 30, ExtendDirection.DOWN)), color=self.middle_panel_color, outline_thickness=self.panel_outline_thickness * self.gui_scale, outline_color=self.panel_outline_color)
    ...
    self.draw_text('Fleet Command', pos=V(title_x, title_y - shadow_offset), font=font, color=shadow_color, origin=origin)
    self.draw_text('Fleet Command', pos=V(title_x, title_y + shadow_offset), font=font, color=shadow_color, origin=origin)
    self.draw_text('Fleet Command', pos=V(title_x - shadow_offset, title_y), font=font, color=shadow_color, origin=origin)
    self.draw_text('Fleet Command', pos=V(title_x + shadow_offset, title_y), font=font, color=shadow_color, origin=origin)
    self.draw_text('Fleet Command', pos=V(title_x, title_y), font=font, color=self.title_text_color, origin=origin)
    ...
    for i, team in enumerate(self.teams):
        self.draw_text(f'{team.name}: {team.type.name} - {len([u for u in self.units.values() if u.team_index == i])}', pos=V(team_info_x, team_info_y + i * (15 * self.gui_scale)), font=self.context_font.new_size(12 * self.gui_scale), color=team.color, origin=Origin.BOTTOMRIGHT)
    if len(self.selected_units_ids) > 0:
        ...
        if selected_count == 1:
            lines = [f'Unit Info: {selected_count}', f'Direction: {avg_direction}°', f'Health: {avg_health}', f'Max Health: {avg_max_health}']
        else:
            lines = [f'Selected Units: {selected_count}', f'Average Direction: {avg_direction}°', f'Average Health: {avg_health}', f'Average Max Health: {avg_max_health}']
        for i, line in enumerate(lines):
            self.draw_text(line, pos=V(info_x, info_y - i * line_height), font=self.context_font.new_size(12 * self.gui_scale), color=Color(200, 200, 200), origin=Origin.TOPLEFT)
    else:
        mouse_world_x, mouse_world_y = self.camera.deduce(self.mouse_pos.x, self.mouse_pos.y)
        closest_unit_index = -1
        closest_unit_index_selectable = -1
        ...
        if closest_unit_index_selectable != -1:
            ...
            info_y = self.extend(self.screen_bottom, 95, ExtendDirection.UP)
            line_height = 15 * self.gui_scale
            lines = [f'Unit Info:', f'Team: {self.teams[unit.team_index].name}', f'Direction: {round(unit.direction)}°', f'Health: {round(unit.health)}', f'Max Health: {round(unit.max_health)}']
            for i, line in enumerate(lines):
                self.draw_text(line, pos=V(info_x, info_y - i * line_height), font=self.context_font.new_size(12 * self.gui_scale), color=Color(200, 200, 200), origin=Origin.TOPLEFT)

def draw_fps(self):
    fps = 0 if self.deltatime == 0 else round(1 / self.deltatime)
    self.draw_text(str(fps), pos=V(self.extend(self.screen_left, 7, ExtendDirection.RIGHT), self.extend(self.screen_top, 2, ExtendDirection.DOWN)), font=self.title_font.new_size(20 * self.gui_scale), color=Color(100, 100, 100), origin=Origin.TOPLEFT)
